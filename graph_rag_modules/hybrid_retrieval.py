...
class HybridRetrievalModule:
    ...
    def __init__(self, config, milvus_module, data_module, llm_client):
        ...
        # 图索引模块
        self.graph_indexing = GraphIndexingModule(config=config, llm_client=llm_client)
        self.graph_idnexed = False
    ...
    def _build_graph_index(self):
        ...
        try:
            ...
            # 创建实体键值对
            self.graph_indexing.create_entity_key_value(
                recipes=recipes, ingredients=ingredients, cooking_steps=cooking_steps
            )

            # 创建关系键值对（这里需要从neo4j获取关系数据）
            relationships = self._extract_relationships_from_graph()
            self.graph_indexing.create_relation_key_values(relationships=relationships)

            # 去重优化
            self.graph_indexing.deduplicate_entities_and_relations()

            self.graph_idnexed = True
    ...
    def extract_query_keywords(self, query: str) -> Tuple[List[str], List[str]]:
        ...
        """
        try:
            reponse = self.llm_client.chat.completions.create(
                model=self.config.llm_model,
                messages=[{"role": "user", "content": prompt}],
                temperature=0.1,
                max_tokens=500,
            )
            result = json.loads(reponse.choices[0].message.content.strip())
    ...
    def entity_level_retrieval(
        ...
        for keyword in entity_keywords:
            ...
            for entity in entities:
                ...
                neighbors = self._get_node_neighbors(
                    entity.metadata["node_id"], max_neighbors=2
                )
        ...
        if len(results) < top_k:
            neo4j_results = self._neo4j_entity_level_search(
                entity_keywords, top_k - len(results)
            )
    ...
    def _get_node_neighbors(self, node_id: str, max_neighbors: int = 3) -> List[str]:
        ...
        try:
            with self.driver.session() as session:
                ...
                """
                result = session.run(
                    query, {"node_id": node_id, "limit": max_neighbors}
                )
    ...
    def topic_level_retrieval(
        ...
        for keyword in topic_keywords:
            ...
            for relation in relations:
                # 获取相关实体信息
                source_entity = self.graph_indexing.entity_kv_store.get(
                    relation.source_entity
                )
                target_entity = self.graph_indexing.entity_kv_store.get(
        ...
        if len(results) < top_k:
            neo4j_results = self._neo4j_topic_level_search(
                topic_keywords, top_k - len(results)
            )
            results.extend(neo4j_results)
        # 4. 按相关性排序并返回
        results.sort(key=lambda x: x.relevance_score, reverse=True)
    ...
    def dual_level_retrieval(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        # 1.提取关键词
        entity_keywords, topic_keywords = self.extract_query_keywords(query=query)

        # 2. 执行双层检索
        enetity_results = self.entity_level_retrieval(
            entity_keywords=entity_keywords, top_k=top_k
        )
        topic_results = self.topic_level_retrieval(
            topic_keywords=topic_keywords, top_k=top_k
        )

        # 3. 结果合并和排序
        all_results = enetity_results + topic_results
    ...
    def vector_search_enhanced(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        try:
            # 使用Milvus进行向量检索
            vector_docs = self.milvus_module.similarity_search(query=query, k=top_k * 2)

            # 用图信息增强并转换为Document对象
            enhanced_docs = []
            for result in vector_docs:
                ...
                if node_id:
                    # 从图中获取邻居信息
                    neighbors = self._get_node_neighbors(node_id=node_id)
                    if neighbors:
                        # 将邻居信息添加到内容中
    ...
    def hybrid_search(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        # 1.双层（实体+主题检索）
        dual_docs = self.dual_level_retrieval(query=query, top_k=top_k)

        # 2. 增强向量检索
        vector_docs = self.vector_search_enhanced(query=query, top_k=top_k)
        ...
        for i in range(max_len):
            ...
            if i < len(dual_docs):
                ...
                if doc_id not in seen_doc_ids:
                    ...
                    # 设置统一的final_sscore字段
                    doc.metadata["final_score"] = doc.metadata.get(
                        "relevance_score", 0.0
                    )
            ...
            if i < len(vector_docs):
                ...
                if doc_id not in seen_doc_ids:
                    ...
                    similarity_score = (
                        max(0.0, 1.0 - vector_score) if vector_score <= 1.0 else 0.0
                    )
        ...
        logger.info(
            f"Round-robin合并：从总共{origin_len}个结果合并为{len(final_docs)}个文档"
        )
