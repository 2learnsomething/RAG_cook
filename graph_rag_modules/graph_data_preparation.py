...
class GraphDataPreparationModule:
    ...
    def _connect(self):
        ...
        try:
            self.driver = GraphDatabase.driver(
                uri=self.uri, auth=(self.user, self.password), database=self.database
            )
            ...
            with self.driver.session() as session:
                ...
                if test_result:
                    ...
                    logger.info("Neo4j连接测试失败")
                    raise ValueError("测试数据库失败,请检查连接")
    ...
    def load_graph_data(self) -> List[Dict]:
        ...
        with self.driver.session() as session:
            ...
            ORDER BY r.nodeId
            """
            result = session.run(query=recipes_query)
            self.recipes = []
            ...
            result = session.run(query=ingredients_query)
            self.ingredients = []
            for record in result:
                node = GraphNode(
                    node_id=record["nodeID"],
            ...
            for recipe in self.recipes:
                try:
                    ...
                    if recipe.properties.get("difficulty"):
                        ...
                        if recipe.properties.get("prepTime") or recipe.properties.get(
                            "cookTime"
                        ):
                            time_info = []
                    ...
                    doc = Document(
                        ...
                        metadata={
                            ...
                            "cuisine_type": recipe.properties.get(
                                "cuisineType", "未知"
                            ),
                    ...
                    logger.warning(
                        f"构建菜谱文档失败 {recipe_name} (ID: {recipe_id}): {e}"
                    )
                    continue
