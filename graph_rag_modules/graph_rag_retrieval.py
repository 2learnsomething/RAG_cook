...
class GraphRAGRetrieval:
    ...
    def _build_graph_index(self):
        ...
        try:
            with self.driver.session() as session:
                ...
                result = session.run(query=entity_query)
                ...
                result = session.run(query=relation_query)
                for record in result:
    ...
    def multi_hop_traversal(self, graph_query: GraphQuery) -> List[GraphPath]:
        ...
        try:
            with self.driver.session() as session:
                """构建多跳遍历查询"""
                source_entity = graph_query.source_entities
                target_entity = graph_query.target_entities
    ...
    def graph_rag_search(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        try:
            ...
            if graph_query.query_type in [QueryType.MULTI_HOP, QueryType.PATH_FINDING]:
                # 多跳遍历
                paths = self.multi_hop_traversal(graph_query=graph_query)
                results.extend(self._paths_to_documents(paths, query))

            elif graph_query.query_type == QueryType.SUBGRAPH:
                # 子图提取
                subgraph = self.extract_knowledge_subgraph(graph_query=graph_query)

                # 图结构推理
                reasoning_chains = self.graph_structure_reasoning(
                    subgraph=subgraph, query=query
                )
            ...
            # 3. 图结构相关性排序
            results = self._rank_by_graph_relavance(results, query)
    ...
    def _analyze_query_complexity(self, query: str) -> float:
        ...
        return min(score / len(complexity_indicators), 1)
