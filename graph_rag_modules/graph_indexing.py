...
class GraphIndexingModule:
    ...
    def create_relation_key_values(
        ...
        for i, (source_id, relation_type, target_id) in enumerate(relationships):
            ...
            content_parts = [
                ...
                f"目标实体：{target_entity.entity_name}({source_entity.entity_type})",
            ]
    ...
    def _llm_enhance_relation_keys(
        ...
        try:
            ...
            result = json.load(response.choice[0].message.content.strip())
    ...
    def deduplicate_entities_and_relations(self):
        ...
        for name, entity_ids in name_to_entities.items():
            if len(entity_ids) > 1:
                ...
                logger.info(f"去重完成 - 删除了{len(entities_to_remove)}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_entity_keys": sum(
                len(kv.index_keys) for kv in self.entity_kv_store.values()
            ),
