...
from graph_config import DEFAULT_CONFIG, GraphRAGConfig
from graph_rag_modules import (
    GraphDataPreparationModule,
    MilvusIndexConstructionModule,
    GenerationIntegrationModule,
)
from graph_rag_modules.hybrid_retrieval import HybridRetrievalModule
from graph_rag_modules.graph_rag_retrieval import GraphRAGRetrieval
from graph_rag_modules.intelligent_query_router import IntelligentQueryRouter
...
class AdvancedGraphRAGSystem:
    ...
    def _initialize_retrievers(self, chunks: List = None):
        ...
        logger.info("初始化传统检索器...")
        self.traditional_retrieval.initialize(chunks=chunks)

        logger.info("初始化图RAG检索器...")
        ...
        while True:
            try:
                ...
                result, analysis = self.ask_question_with_routing(
                    user_input, stream=use_stream, explain_routing=explain_routing
                )
    ...
    def _rebuild_knowledge_base(self):
        ...
        # 确认操作
        confirm = (
            input("⚠️  这将删除现有的向量数据并重新构建，是否继续？(y/N): ")
            .strip()
            .lower()
        )
        if confirm != "y":
